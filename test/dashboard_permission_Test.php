...
class Dashboard_Permission_Test extends PHPUnit_Framework_TestCase {
	...
	public function test_dashboard_share_with_is_idempotent() {
		...
		$this->assertEquals($read_perms[0], PermissionQuarkPool_Model::build('user', 'agnes'));
	}
	...
	public function test_dashboard_get_shared_with() {
		...
		$this->assertEquals($read_perm[0], PermissionQuarkPool_Model::build('user', 'no rest for the wicked'));
